...
function darkside(p1,tr){
	...
	if(p1!="" && p1!=null){
		...
		for(var k=1;k<=ccghm;k++){
			...
			}else var out="Desculpe, nenhuma BIN vÃ¡lida inserida, ou a BIN esta incompleta, verifique se o formato estÃ¡ correto, exemplo: '666666xxxxxxxxxx' ";
		}
		if(jou==2) var out=out+"</xml>";
		else if(jou==3) var out=out+"}";
...
function mkCClist(){
	...
	c[i]="4xxx9x604015xxxx"; cd[i]="Visa-Carta SÃ¬-Unipol Banca (Italy)"; i++;
	c[i]="4xxx9x144046xxxx"; cd[i]="Visa-Carta SÃ¬-Banco di Sardegna (Italy)"; i++;
	c[i]="4xxx9xxx40xxxxxx"; cd[i]="Visa-Carta SÃ¬ (Italy)"; i++;
